...
class ForwardSelection:
    ...
    def bestfit(self, X, y):
        ...
        # List of predictors that have not been chosen
        remaining_predictors = X.columns.tolist()

        # Create list to store testing data
        testing_data = []

        def give_score(score_data):
            return score_data[1]

        # Find the total number of iterations in the forward selection algorithm
        total_iterations = sum([len(remaining_predictors) - i for i in range(len(remaining_predictors))])
